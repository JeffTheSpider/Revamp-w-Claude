...
void LED_Special2() {
  ResetClock();
  long randCell, randColour1, randColour2, randColour3;
  while (Mode == "Special2")
  {
...
void LED_Special4() {
  ...
  {
    for (int cell = 0; cell < PixelCount; cell++) {
      // Map each pixel to a hue based on position + offset for rotation
      int hue = ((cell * 256 / PixelCount) + offset) % 256;
      // Convert hue to RGB using a simple wheel function
      int r, g, b;
      if (hue < 85) {
        r = hue * 3;
        g = 255 - hue * 3;
