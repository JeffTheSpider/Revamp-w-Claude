...
static void _notifyPulse(int br) {
  float phase = (float)notify.step * 6.28318f / 30.0f;
...
// Uses showStrip() for lamp's multi-GPIO BitBang output.
bool notifyTick(int br) {
  ...
  showStrip();  // Lamp uses multi-GPIO showStrip() instead of strip.Show()
  notify.step++;
  return true;
}
